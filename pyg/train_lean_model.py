...
from dataset import OSMChengduVFE
...
def main(args, loglevel):
    ...
    if args.wandb_log:
        ...
        if args.pretraining_dataset is not None:
            tags.append(args.pretraining_dataset)
    ...
    data = OSMChengduVFE(args.path,backbone=args.backbone, pretraining_dataset=args.pretraining_dataset)[0]
    ...
    visual_features = 512 if args.backbone == 'ResNet18' else 2048
...
if __name__ == '__main__':
    ...
    # VFE args
    # p.add('--no_img', default=True, action='store_true', help='only use node attributes as features (ignore images)')
    p.add('--backbone', default=None, help='backbone network to use (choose from [resnet18, alexnet, vgg, squeezenet, densenet, inception])')
    p.add('--pretraining_dataset', default=None, help='Which dataset should the backbone be pretrained on [\'ImageNet\', \'NWPU-RESISC45\'].')
